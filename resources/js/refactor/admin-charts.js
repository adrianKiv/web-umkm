(function () {
    function initCharts(cfg) {
        ...
        // Kategori pie
        const kategoriCanvas = document.getElementById(
            "kategoriDistributionChart",
        );
        if (kategoriCanvas && kategoriLabels.length) {
            const pieColors = kategoriLabels.map(
                (_, index) => `hsl(${(index * 48) % 360}, 70%, 60%)`,
            );
            const totalKategori = kategoriValues.reduce(
                (sum, value) => sum + Number(value || 0),
                0,
            );
            const kategoriDisplayLabels = kategoriLabels.map((label, index) => {
                const value = Number(kategoriValues[index] || 0);
                const percent = totalKategori
                    ? ((value / totalKategori) * 100).toFixed(1)
                    : "0.0";
        ...
        // Wilayah doughnut
        const wilayahCanvas = document.getElementById(
            "wilayahDistributionChart",
        );
        if (wilayahCanvas && wilayahLabels.length) {
            const wilayahColors = wilayahLabels.map(
                (_, index) => `hsl(${(index * 46 + 210) % 360}, 75%, 60%)`,
            );
            const totalWilayah = wilayahValues.reduce(
                (sum, value) => sum + Number(value || 0),
                0,
            );
            const wilayahDisplayLabels = wilayahLabels.map((label, index) => {
                const value = Number(wilayahValues[index] || 0);
                const percent = totalWilayah
                    ? ((value / totalWilayah) * 100).toFixed(1)
                    : "0.0";
                return `${label} (${percent}%)`;
            });
        ...
        if (jamBukaCanvas && jamBukaLabels.length) {
            ...
            new Chart(jamBukaCanvas, {
                type: "bar",
                data: {
                    labels: jamBukaLabels,
                    datasets: [
                        {
                            label: "Jumlah UMKM",
                            data: jamBukaValues,
                            backgroundColor: jamBukaColors,
                            borderColor: jamBukaColors.map((c) =>
                                c.replace("0.7", "1"),
                            ),
                            borderWidth: 1,
                            borderRadius: 8,
