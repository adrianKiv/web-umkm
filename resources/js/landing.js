...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const mobileFilterModalEl = document.getElementById(
        "mobileLandingFilterModal",
    );
    const openMobileFilterBtn = document.getElementById("openMobileFilterBtn");
    const mobileFilterApplyBtn = document.getElementById(
        "mobileFilterApplyBtn",
    );
    const mobileFilterResetBtn = document.getElementById(
    ...
    const getDesktopFilterSelect = (name) =>
        desktopForm?.querySelector(`select[name="${name}"]`) || null;
    const getMobileHidden = (name) =>
        mobileForm?.querySelector(`input[type="hidden"][name="${name}"]`) ||
        null;
    ...
    const syncModalFilterInputs = () => {
        ...
        if (kelompokSelect)
            kelompokSelect.value = readInitialFilterValue("id_kelompok");
        if (kategoriSelect)
            kategoriSelect.value = readInitialFilterValue("id_kategori");
        if (ratingSelect)
            ratingSelect.value = readInitialFilterValue("min_rating");
    };
    ...
    mobileFilterApplyBtn?.addEventListener("click", function () {
        ...
        syncFilterValue(
            "id_kategori",
            document.getElementById("mobileFilterKategori")?.value || "",
        );
        syncFilterValue(
            "min_rating",
            document.getElementById("mobileFilterRating")?.value || "",
        );
    ...
    const detailModal =
        detailModalEl && typeof bootstrap !== "undefined"
    ...
    document.addEventListener("click", function (event) {
        ...
        fetch(detailUrl, {
            ...
            .catch(() =>
                setDetailError("Gagal memuat detail UMKM. Silakan coba lagi."),
            );
    });

    const preferenceModalEl = document.getElementById("preferenceModal");
    const preferenceModal =
        preferenceModalEl && typeof bootstrap !== "undefined"
    ...
    const preferenceSubmitButton = preferenceForm?.querySelector(
        'button[type="submit"]',
    );
    const preferenceCheckboxes = preferenceForm
        ? Array.from(
              preferenceForm.querySelectorAll('input[name="kategori_ids[]"]'),
          )
        : [];
    const maxSelection = Number(preferenceModalEl?.dataset.maxSelection || 3);

    const getSelectedPreferences = () =>
        preferenceCheckboxes.filter((checkbox) => checkbox.checked);
    ...
    const updatePreferenceState = () => {
        ...
        preferenceCheckboxes.forEach((checkbox) => {
            checkbox.disabled =
                !checkbox.checked && selected.length >= maxSelection;
        });

        if (preferenceSubmitButton) {
            preferenceSubmitButton.disabled =
                selected.length === 0 || selected.length > maxSelection;
        }
    ...
    preferenceForm?.addEventListener("submit", async function (event) {
        ...
        const selectedIds = getSelectedPreferences().map(
        ...
        try {
            ...
            if (preferenceError) {
                preferenceError.textContent =
                    "Gagal menyimpan preferensi. Silakan coba lagi.";
                preferenceError.classList.remove("d-none");
            }
        }
    });
...
function performLiveSearch(query, shouldScroll = false) {
    // Tambah parameter shouldScroll
    ...
    if (headerSearchForm) {
        const kelompokSelect = headerSearchForm.querySelector(
            'select[name="id_kelompok"]',
        );
        const kategoriSelect = headerSearchForm.querySelector(
            'select[name="id_kategori"]',
        );
        const ratingSelect = headerSearchForm.querySelector(
    ...
    fetch(url.toString(), {
        ...
        .then((html) => {
            ...
            const newRecommendedSection = newDoc.querySelector(
                ".recommended-section",
            );
            const currentRecommendedSection = document.querySelector(
                ".recommended-section",
            );

            if (currentRecommendedSection) {
                if (newRecommendedSection) {
                    currentRecommendedSection.innerHTML =
                        newRecommendedSection.innerHTML;
                } else {
                ...
                if (currentSection && currentSection.parentNode) {
                    const recommendedClone =
                        newRecommendedSection.cloneNode(true);
...
function scrollToResults() {
    const targetSection =
        ...
        document.querySelector(".umkm-section");
    if (targetSection) {
